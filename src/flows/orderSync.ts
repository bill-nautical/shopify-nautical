  ...
  onExecution: async (context, params) => {
    ...
    try {
      if (orderData) {
        ...
        await processOrderWebhook(shopifyConnection, nauticalConnection, orderData);
        
        logInfo(context, `Successfully processed order webhook: ${orderData.id}`, {
          orderId: orderData.id
        });
      } else {
        // Schedule-based sync for all orders in a time period
        const syncResult = await syncAllOrders(shopifyConnection, nauticalConnection);
...
async function processOrderWebhook(shopifyConn: any, nauticalConn: any, orderData: any) {
  ...
  for (const order of recentOrders) {
    try {
      ...
      if (existingOrder) {
        ...
        if (existingOrder.status !== mapShopifyStatusToNautical(order.displayFinancialStatus)) {
...
function transformShopifyOrderToNautical(shopifyOrder: any) {
  // Extract line items
  const lineItems = shopifyOrder.lineItems?.edges?.map((edge: any) => {
