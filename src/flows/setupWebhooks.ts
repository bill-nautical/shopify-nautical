...
export const setupWebhooksFlow = flow({
  ...
  description: "Set up webhooks for Shopify to notify of product, inventory, and order changes",
...
async function registerWebhook(connection: any, webhook: any) {
  ...
  try {
    ...
    if (response.data.data.webhookSubscriptionCreate.userErrors.length > 0) {
      throw new Error(response.data.data.webhookSubscriptionCreate.userErrors[0].message);
    }
