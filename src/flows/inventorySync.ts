...
async function fetchShopifyInventory(connection: any) {
  ...
  try {
    ...
    const inventoryItems = response.data.data.inventoryItems.edges.map(
      (edge: any) => {
...
function calculateInventoryUpdates(
  ...
  for (const item of shopifyInventory) {
    if (item.sku) {
      ...
      const totalQuantity = item.levels.reduce(
...
async function applyInventoryUpdates(
  ...
  for (const update of updates) {
    try {
      ...
      const result = await updateNauticalInventory(
        nauticalConn,
